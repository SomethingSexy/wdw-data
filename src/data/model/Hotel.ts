...
const RAW_HOTEL_ATTRIBUTES = ['tier'];
...
export const normalizeHotel = location => ({
  ...pick(location, RAW_LOCATION_ATTRIBUTES),
  address: location.Address || null,
  areas: location.Areas.map(area => area.name),
  tier: location.Hotel.tier
});

/**
 * TODO: There is shared stuff between Hotel and Park that we need to combine
 */
class HotelModel {
  ...
  public async load(include?: GetTypes[]) {
    const { Activity, Address, Area, Hotel, Location } = this.dao;
    // setting to any because I am not gonna repeat sequelize's api
    // TODO: We need to add rooms here
    const queryInclude: any[] = [{
      ...
      as: 'Hotel',
      attributes: RAW_HOTEL_ATTRIBUTES,
      model: Hotel
    }];
  ...
  public async upsert (item: ILocation, transaction?) {
    ...
    this.logger('debug', `Adding/updating hotel ${item.extId}.`);
    ...
    this.logger('debug', `Finished adding/updating hotel ${item.extId}.`);
